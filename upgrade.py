...
# 先把升级包拷贝到当前目录，然后更改此变量为包名
upgrade_pack = "PARK_A04_01_20170125A1.bin"
...
def run_cmd(ip, cmd):
    ...
    try:
        outs, errs = p.communicate(timeout=30*10)
...
def do_action(space_code, ip):
    try:
        global upgrade_pack
        outs = ''
        #copy2device(ip, "platform_set.cfg", '/mnt/nand/platform_set.cfg')
        outs += httpcfg(ip, "timefrequency=-1&datestampenable3=2&sntpip=192.168.80.76")
        outs += httpcfg(ip, "language=ie&bitrate1=2000")
        outs += update(ip, upgrade_pack)
        #outs += run_cmd(ip, r"sed -i 's/<is_plate_numble>1<\/is_plate_numble>/<is_plate_numble>0<\/is_plate_numble>/' /config/dsp_config.xml")
        outs += run_cmd(ip, r"sync && /sbin/reboot -f &")
        return outs
    except Exception as e:
        #print("\033[91m" + ip + " !!!FAILED!!!" + "\033[0m", "\033[93m", e, "\033[0m")
